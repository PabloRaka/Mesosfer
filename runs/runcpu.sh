...
python -m scripts.train.tok_train --max-chars=2000000000
...
python -m scripts.train.base_train \
    ...
    --num-iterations=5000 \
    --run=$WANDB_RUN
python -m scripts.eval.base_eval --device-batch-size=1 --split-tokens=16384 --max-per-task=16
...
# python -m scripts.chat.chat_cli -p "What is the capital of France?"
