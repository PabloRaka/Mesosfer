...
def get_torch_accelerator_backend() -> str:
    if torch.cuda.is_available():
        return "rocm" if is_rocm_torch() else "cuda"
...
def _detect_compute_dtype():
    ...
    if torch.cuda.is_available():
        if is_rocm_torch():
            ...
            # MI300X support bf16, which is the default compute dtype we want.
            return torch.bfloat16, f"auto-detected: ROCm {torch.version.hip} (bf16 default)"
...
def autodetect_device_type():
    # prefer to use CUDA/ROCm if available, otherwise use MPS, otherwise fallback on CPU
    if torch.cuda.is_available():
        device_type = "cuda"
        ...
        device_type = "cpu"
    accelerator_backend = get_torch_accelerator_backend()
    if accelerator_backend in {"cuda", "rocm"}:
        print0(f"Autodetected device type: {device_type} ({accelerator_backend})")
    else:
...
def compute_init(device_type="cuda"): # cuda|cpu|mps
    ...
    if is_ddp_requested:
        if device_type != "cuda":
            # DDP env variables are set (torchrun) but device is not CUDA.
            # This silently disables DDP, which would give wrong results.
            logger.warning(
                "DDP requested (RANK/WORLD_SIZE set) but device_type='%s' is not CUDA. "
                "DDP WILL NOT BE INITIALIZED. Running single-process only.",
                device_type,
            )
